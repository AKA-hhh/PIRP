...
class Diffusion(object):
    def __init__(self, args, config, device=None):
        ...
        self.lpips_model = lpips.LPIPS(net='alex', spatial=False).to(torch.device("cuda"))
    ...
    def pirp_wrapper(self, model, cls_fn, logger):
        ...
        if deg == 'cs_walshhadamard':
            ...
            if args.operator_imp == 'SVD':
                ...
                A_funcs = SuperResolution(config.data.channels, self.config.data.image_size, blur_by, self.device)
            else:
        ...
        img_ind = -1
        # Create directories for saving generated and original images
        ...
        for x_orig, classes in pbar:
            ...
            # SSIM calculation
            x_ssim = inverse_data_transform(config, x[0]).to(self.device)
            orig_ssim = inverse_data_transform(config, x_orig).to(self.device)
            ssim_val = ssim(x_ssim, orig_ssim, data_range=1.0).item()
            avg_ssim += ssim_val

            # Save images and calculate PSNR
            for j in range(x[0].size(0)):
                # Save generated image
                tvu.save_image(
                    x_ssim[j], 
                    os.path.join(generated_dir, f"{idx_so_far + j}.png")
                )

                # Calculate PSNR
                mse = torch.mean((x_ssim[j].to(self.device) - orig_ssim[j]) ** 2)
                psnr = 10 * torch.log10(1 / mse)
                avg_psnr += psnr

                # Log metrics
                logger.info("img_ind: %d, PSNR: %.2f, LPIPS: %.4f, SSIM: %.4f" % 
                           (img_ind, psnr, lpips_final, ssim_val))

            idx_so_far += y.shape[0]

            logger.info("Avg PSNR: %.2f, Avg LPIPS: %.4f, Avg SSIM: %.4f     (** After %d iteration **)" % 
                       (avg_psnr / (idx_so_far - idx_init), 
                        avg_lpips / (idx_so_far - idx_init), 
                        avg_ssim / (idx_so_far - idx_init),
                        idx_so_far - idx_init))
