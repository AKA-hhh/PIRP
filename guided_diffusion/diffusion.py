...
class Diffusion(object):
    ...
    def pirp_wrapper(self, model, cls_fn, logger):
        ...
        avg_psnr = 0.0
        avg_lpips = 0.0
        avg_ssim = 0.0  # 新增SSIM统计变量
        pbar = tqdm.tqdm(val_loader)

        img_ind = -1
        generated_dir = os.path.join(self.args.image_folder, "generated")
        original_dir = os.path.join(self.args.image_folder, "original")
        ...
        for x_orig, classes in pbar:
            ...
            if True:
                ...
                for i in range(len(Apy)):
                    ...
                    os.path.join(original_dir, f"orig_{idx_so_far + i}.png")
                    )
            ...
            # coef1 = extract_and_expand(np.sqrt(alphas_cumprod), 999, x.cpu())
            # coef2 = extract_and_expand(np.sqrt(1.0 - alphas_cumprod), 999, x.cpu())
            # coef1 = coef1.to(self.device)
            # coef2 = coef2.to(self.device)
            ...
            with torch.no_grad():           
                ...
                #x, _ = pirp_diffusion_tom(x, model, self.betas, A_funcs, y, sigma_y, cls_fn=cls_fn, classes=classes, config=config, args=args)
            ...
            for j in range(x[0].size(0)):
                tvu.save_image(
                    x[0][j], os.path.join(generated_dir, f"{idx_so_far + j}.png")
                )
                orig = inverse_data_transform(config, x_orig[j])
                recon = x[0][j].to(self.device)  # 确保使用相同设备
                mse = torch.mean((x[0][j].to(self.device) - orig) ** 2)
                psnr = 10 * torch.log10(1 / mse)
                 # 计算SSIM（新增部分）
                ssim_val = ssim(
                    orig.unsqueeze(0),  # 增加batch维度
                    recon.unsqueeze(0), 
                    data_range=1.0,     # 假设图像范围是[0,1]
                    nonnegative_ssim=True
                )
                avg_ssim += ssim_val.item()
                 # 更新日志输出
                logger.info(f"img_ind: {img_ind}, PSNR: {psnr:.2f}, LPIPS: {lpips_final:.4f}, SSIM: {ssim_val.item():.4f}")
                avg_psnr += psnr
            ...
            # 更新进度条描述
            logger.info(
                f"Avg PSNR: {avg_psnr/(idx_so_far-idx_init):.2f}, "
                f"LPIPS: {avg_lpips/(idx_so_far-idx_init):.4f}, "
                f"SSIM: {avg_ssim/(idx_so_far-idx_init):.4f} "
                f"(** After {idx_so_far-idx_init} **)"
            )
        ...
        # 最终输出结果
        print(f"Total Average PSNR: {avg_psnr/(idx_so_far-idx_init):.2f}")
        print(f"Total Average LPIPS: {avg_lpips/(idx_so_far-idx_init):.4f}")
        print(f"Total Average SSIM: {avg_ssim/(idx_so_far-idx_init):.4f}")  # 新增输出
        print(f"Number of samples: {idx_so_far-idx_init}")
